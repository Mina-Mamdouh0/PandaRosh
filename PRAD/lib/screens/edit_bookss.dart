...
class EditButtonBookss extends StatefulWidget {
  const EditButtonBookss({Key? key}) : super(key: key);

  @override
  _EditButtonBookssState createState() => _EditButtonBookssState();
}

class _EditButtonBookssState extends State<EditButtonBookss> {
  ...
  Future<void> getDataa() async {
    ...
    try {
      ...
      movieID = prefs.getString('bookID').toString();

      //movieIDBara = prefs.getString('bookIDBara').toString();
      ...
      await http.get(url).then((value) async {
        if (value.body == "{}") {
          ...
          final urlCat = Uri.parse(
              '${'https://pandarosh-91270-default-rtdb.firebaseio.com/categories/' +
                  extractedData['category']}/name.json');

          //get category name
          await http.get(urlCat).then((valueCat) async {
            //get writer name
            final urlWrit = Uri.parse(
                '${'https://pandarosh-91270-default-rtdb.firebaseio.com/writers/' +
                    extractedData['writer']}/name.json');
  ...
  void uploadN() async {
    try {
      final url = Uri.parse(
          'https://pandarosh-91270-default-rtdb.firebaseio.com/books.json?orderBy="info"&equalTo="$movieID"');

      Uri urll = Uri.https(
          'pandarosh-91270-default-rtdb.firebaseio.com', '/booksInfo/$movieID.json');
